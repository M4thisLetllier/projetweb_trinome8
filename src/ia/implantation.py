...
def get_pdc_data(id_pdc):
    """Récupère les données du PDC depuis la base MySQL"""
    conn = None
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor(dictionary=True)

        # 1. Requête principale : Infos du PDC, de la Station, de l'accès et de l'accessibilité
        query_main = """
            SELECT
        ...
        # On initialise tout à 0, puis on passe à 1 si on trouve le mot clé dans les listes

        #chercher des mots-clés dans les listes
        ...
        caracteristiques = {
            ...
            # Attributs non présents dans le MCD fourni, on garde des valeurs par défaut (ou celles que tu avais mises)
            "reservation": 0,
            ...
            "puissance_nominale": float(pdc_info['puissance_nominale']),
            "nbre_pdc": int(pdc_info['nbre_pdc']),
...
def main():
    try:
        ...
        modele_path = os.path.join(script_dir, 'modele_prediction_implantation.pkl')
    ...
    try:
        # 2. Récupération de l'ID envoyé par PHP
        ...
        # 3. Récupération des données depuis MySQL
        ...
        print(json.dumps({
            ...
            "prediction_IA": str(prediction)
        }))

    except Exception as e:
        print(json.dumps({"erreur": f"Erreur d'exécution : {str(e)}"}))
